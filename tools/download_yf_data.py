...
def download_instrument_data(ticker, instrument_name, start_date, end_date, instrument_start_year=None):
    """Download historical data for a single instrument"""

    # Override start date if Start Date column exists
    if instrument_start_year and instrument_start_year.strip():
        try:
            year = int(instrument_start_year.strip())
    ...
    end_api = (datetime.strptime(end_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    try:
        print(f"Downloading {ticker} ({instrument_name}) from {start_date}...")
