...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: python compute_match_wavelet.py <asset_class>")
        sys.exit(1)
    ...
    SCALES = np.geomspace(30, 160, 150)  # Logarithmic spacing (empirically optimized)

    # Output files
    final_output_path = os.path.join(OUTPUT_DIR, f"match_wavelet_results_{asset_class}.csv")
    log_path = os.path.join(OUTPUT_DIR, "wavelet_processing_log.csv")
    ...
    # Initialize final output CSV with header
    ...
    print(f"Starting wavelet computation and cycle matching for {asset_class.upper()} instruments...")
    print(f"Period range: {MIN_PERIOD}-{MAX_PERIOD} days")
    print(f"Wavelet: {WAVELET}, Scales: {SCALES[0]:.1f}-{SCALES[-1]:.1f} ({len(SCALES)} points)")
    ...
    for inst in instruments:
        ...
        log_entry(timestamp, asset_class, category, instrument_name, ticker, status, message, log_path)
...
def log_entry(timestamp, asset_class, category, instrument, ticker, status, message, log_path):
...
def scale_to_period(scales, wavelet_name, dt=1):
    """Convert wavelet scales to periods"""
    if 'cmor' in wavelet_name:
        # Extract parameters from wavelet name (e.g., 'cmor1.5-1.0')
        params = wavelet_name[4:].split('-')
        bandwidth = float(params[0])
        center_freq = float(params[1])
        return scales * (4 * np.pi) / (center_freq + np.sqrt(2 + center_freq**2)) * dt
    else:
        # Default conversion for general wavelets
        return scales * dt

def find_dominant_cycles_wavelet(data, cycle_table, min_period, max_period, wavelet, scales):
    """Find dominant cycles using wavelet transform"""
    # Compute Continuous Wavelet Transform
    coefficients, frequencies = pywt.cwt(data, scales, wavelet, sampling_period=1)
    
    # Calculate wavelet power spectrum
    power = (abs(coefficients)) ** 2
    mean_power = np.mean(power, axis=1)  # Average power over time
    ...
    filtered_power = mean_power[period_mask]
    
    if len(filtered_power) == 0:
        return []
    
    # Normalize power
    filtered_power = filtered_power / np.max(filtered_power)
    
    # Find peaks in the wavelet power spectrum
    peak_indices, _ = find_peaks(filtered_power, height=0.1, distance=5)
